...
The user supplied the missing landmark: the arrow in `sample document` should point at "a known heading".
...
Verified visually: the arrow lands on "a known heading", the whole annotation sits on the page,
...
| ---------- | -------------- | ------ | --------------- | --------------------- |
| doc A      | A4 595x841.9   | 0.3178 | 299..562 of 595 | 164..274 of 841.9     |
...
The user highlighted text on the device and re-synced. Re-fetching the sample document's document index
...
Fixture at `~/Desktop/remarkable-test-fixtures/sample-document-v2.rmdoc`, rendered result at
...
`sample document` is exactly five files: `.content`, `.metadata`, `.pagedata`, `.pdf`, and one `.rm`.
