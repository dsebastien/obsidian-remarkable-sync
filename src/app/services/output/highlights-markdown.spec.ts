...
describe('normaliseHighlightText', () => {
     ...
     * "...(Okta Device" + "Trust, ..." arrived as "DeviceTrust".
     */
    test('repairs a line join at a case transition', () => {
        const out = normaliseHighlightText('using MDM (Okta DeviceTrust, OS-update', [90, 90])
        expect(out).toContain('Okta Device Trust')
    })

    test('repairs a join after a full stop', () => {
        const out = normaliseHighlightText('upcoming migration.Deployed Tailscale now', [95, 95])
        expect(out).toContain('migration. Deployed')
    })
    ...
    test('never splits camelCase with a short leading run', () => {
        const out = normaliseHighlightText(
            'Responsible for the Apple fleet (macOS/iOS) using Kandji and Iru MDM today',
            [180, 180]
        )
    ...
    test('never splits iPhone', () => {
        const out = normaliseHighlightText(
            'policies and self-service with 120+ iPhone restrictions applied here',
            [170, 170]
        )
    ...
    test('leaves an ambiguous lowercase join alone rather than guessing wrong', () => {
        // "Windowsmachines" could equally be "Window smachines" without a
        // dictionary, so it is left intact.
        const out = normaliseHighlightText('and Windowsmachines in Intune', [70, 70])
        expect(out).toContain('Windowsmachines')
        expect(out).not.toContain('Window smachines')
    })
